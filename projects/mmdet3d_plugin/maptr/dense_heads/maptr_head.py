...
class MapFusionTRHead2(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        super(MapFusionTRHead2, self).__init__(
    ...
    def forward(self, map_feature, map_mask, prev_bev=None,  only_bev=False):
        bs = map_feature.shape[0]
        dtype = map_feature[0].dtype
...
@HEADS.register_module()
class MapFusionTRHead(DETRHead):
    """Head of Detr3D.
    Args:
        with_box_refine (bool): Whether to refine the reference points
            in the decoder. Defaults to False.
    ...
    def __init__(self,
        ...
        super(MapFusionTRHead, self).__init__(
            *args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)
        self.loss_pts = build_loss(loss_pts)
    ...
    @force_fp32(apply_to=('mlvl_feats', 'prev_bev'))
    def forward2(self, map_feature, map_mask, prev_bev=None,  only_bev=False):
        bs = map_feature.shape[0]
        dtype = map_feature[0].dtype
        # import pdb;pdb.set_trace()
        if self.query_embed_type == 'all_pts':
