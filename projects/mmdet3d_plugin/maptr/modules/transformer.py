...
class MapFusionTRPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 modality='vision',
                 **kwargs):
        super(MapFusionTRPerceptionTransformer, self).__init__(**kwargs)
        if modality == 'fusion':
            self.fuser = build_fuser(fuser) #TODO
        self.use_attn_bev = encoder['type'] == 'BEVFormerEncoder'
        self.encoder = build_transformer_layer_sequence(encoder)
    ...
    def forward(self,
        ...
        inter_states, inter_references = self.decoder(
            query=query, #torch.Size([1000, 4, 256])
            key=None,
            value=bev_embed, #torch.Size([20000, 4, 256])
            query_pos=query_pos, #torch.Size([1000, 4, 256])
            reference_points=reference_points,#torch.Size([4, 1000, 2])
